...
if(isset($_GET['removeValidPayment']))
{
	...
	$id_order = $_POST['id_order'];
	startTransection();
	if(dbQuery("DELETE FROM tbl_payment WHERE id_order = ".$id_order." AND valid = 1") == FALSE)
	{
		$sc = FALSE;
		$message = 'ลบรายการไม่สำเร็จ';
	}

	if(dbQuery("UPDATE tbl_order SET valid = 0 WHERE id_order = ".$id_order) == FALSE)
	{
		$sc = FALSE;
		$message = 'เปลี่ยนสถานะการชำระเงินไม่สำเร็จ';
	}
